...
export function RunInsightCard({ runId }: { runId: string }) {
  ...
  const ready =
    insight?.ai_status === "completed" && Boolean(insight.ai_summary?.trim());
  ...
  return (
    <section className="surface-elev overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-gray-100 border-b px-4 py-4 sm:px-5">
        ...
        <button
          className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-[color:var(--moby-600)] px-3.5 font-semibold text-[12px] text-white transition-colors hover:bg-[color:var(--moby-800)] disabled:cursor-not-allowed disabled:opacity-70"
          disabled={generating}
      ...
      <div className="px-5 py-4">
        {error ? (
          <p className="text-[13px] text-[color:var(--ink-4)] leading-6">
            {error}
          </p>
        ) : ready ? (
