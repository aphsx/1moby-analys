...
export function LifecycleMixCard({
  overview,
}: {
  overview: DashboardOverview;
}) {
  const lifecycleEntries = Object.entries(overview.lifecycle) as [
  ...
  return (
    <section className="surface-elev flex h-full flex-col overflow-hidden">
      <div className="flex min-w-0 items-start justify-between gap-4 border-gray-100 border-b px-4 py-3 sm:px-5">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex min-w-0 items-baseline gap-1.5">
            ...
            <span className="type-muted font-medium text-[12px] leading-none">
              customers
            </span>
          </div>
        </div>
        <div className="flex shrink-0 flex-col items-end gap-2">
          <span className="type-meta rounded-full bg-gray-50 px-3 py-1 font-normal text-[11px]">
            4 segments
          </span>
          <span className="type-meta rounded-full bg-gray-50 px-3 py-1 font-normal text-[11px]">
            Scale 0-{chartMaxPct}%
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <div className="grid min-w-0 flex-1 grid-cols-[32px_minmax(0,1fr)] gap-3">
          <div className="type-muted grid h-[228px] grid-rows-[auto_1fr_auto] text-right font-normal text-[10px]">
          ...
          <div
            aria-label="Lifecycle distribution vertical bar chart"
              ...
              {lifecycleStats.map(({ stage, count, pct }) => (
                ...
                >
                  <div className="mb-2 rounded-full bg-gray-50 px-2 py-1 text-center">
                    <div className="num text-[11px] text-[color:var(--ink-1)] tabular-nums leading-none">
                      {formatNumber(count)}
                    </div>
                  </div>
        ...
        <div className="mt-3 grid grid-cols-4 gap-3 sm:gap-4">
          {lifecycleStats.map(({ stage, pct }) => (
            <div className="min-w-0 text-center" key={stage}>
              <div
                className={`type-muted truncate font-normal text-[10px] uppercase tracking-[.08em] ${TEXT_SAFE}`}
              >
                {stage}
              </div>
