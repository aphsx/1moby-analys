...
const CHANNEL_SHARES = [
  { color: "#8b5cf6", key: "bc_usage_share", label: "BC" },
  { color: "#06b6d4", key: "api_usage_share", label: "API" },
  { color: MOBY_BRAND.orangeWarm, key: "otp_usage_share", label: "OTP" },
] as const;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <DateFact
          hint={`${snapshot.customer_age_days.toLocaleString()} วัน`}
          label="Joined"
          value={formatDate(snapshot.join_date)}
        />
        <DateFact
          hint={daysAgo(snapshot.last_access) ?? "—"}
          label="Last access"
          value={formatDate(snapshot.last_access)}
        />
        <DateFact
          hint={daysAgo(snapshot.last_send) ?? "—"}
          label="Last send"
          value={formatDate(snapshot.last_send)}
        />
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="mb-3 flex items-center justify-between">
          <p className="font-semibold text-[11px] text-[color:var(--ink-5)] uppercase tracking-[.12em]">
            Usage share · 180d
          </p>
        ...
        <div className="space-y-2.5">
          {CHANNEL_SHARES.map((c) => {
            ...
            return (
              <div className="flex items-center gap-3" key={c.key}>
                <span className="w-9 shrink-0 font-semibold text-[11px] text-[color:var(--ink-4)]">
                  {c.label}
                </span>
                ...
                <span className="num w-10 shrink-0 text-right font-semibold text-[11.5px] text-[color:var(--ink-2)]">
                  {(share * 100).toFixed(0)}%
                </span>
              </div>
            );
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {status ? (
          <span
            className="inline-flex h-[20px] items-center rounded-full px-2 font-semibold text-[10px] text-white"
      ...
      <p className="num mt-2 font-semibold text-[18px] text-[color:var(--ink-1)]">
        {credit.toLocaleString()}
      </p>
      <p className="text-[10.5px] text-[color:var(--ink-5)]">
        credits · หมดอายุ {formatDate(expire)}
      </p>
    </div>
  );
}
...
function DateFact({
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3">
      ...
      <p className="num mt-1 font-semibold text-[12.5px] text-[color:var(--ink-1)]">
        {value}
      </p>
