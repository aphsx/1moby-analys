...
async function main() {
  ...
  if (!path) {
    throw new Error("Usage: bun run seed-import.ts <path-to-xlsx>");
  }
  ...
  console.log(
    "[seed] train source ready:",
    train.source_id,
    JSON.stringify(trainClean).slice(0, 200)
  );
  ...
  console.log(
    "[seed] predict source ready:",
    predict.source_id,
    JSON.stringify(predictClean).slice(0, 200)
  );
