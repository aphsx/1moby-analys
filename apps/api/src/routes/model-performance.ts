...
function buildCompetition(card: ModelCard): CandidateResult[] | undefined {
  ...
  for (const entry of card.candidate_selection ?? []) {
    if (entry.candidate) {
      selection.set(entry.candidate, entry);
    }
  }

  const results: CandidateResult[] = Object.entries(scores).map(
    ([algorithm, cvScore]) => {
...
function averageMetrics(rows: EvaluationRow[]): Record<string, number> {
  ...
  for (const row of rows) {
    for (const [key, value] of Object.entries(asMetrics(row.metricsJson))) {
      ...
      const entry = sums.get(key) ?? { n: 0, total: 0 };
      entry.total += value;
      entry.n += 1;
      sums.set(key, entry);
    }
...
async function buildEntry(
  ...
  const [champion] = await db
    ...
    .innerJoin(
      mlModelVersions,
      eq(mlModelAliases.modelVersionId, mlModelVersions.id)
    )
    .where(
      and(
        eq(mlModelAliases.alias, "production"),
        eq(mlModelAliases.modelType, modelType)
      )
    )
    .limit(1);
  if (!champion) {
  ...
  for (const split of ["validation", "test"] as const) {
    ...
    if (row) {
      splits.push({ metrics: asMetrics(row.metricsJson), split });
    }
  }
  if (backtests.length > 0) {
    splits.push({ metrics: averageMetrics(backtests), split: "backtest_avg" });
  }

  const baselines = evaluations
    .filter((row) => row.baselineName !== null && row.datasetSplit === "test")
    .map((row) => ({
      metrics: asMetrics(row.metricsJson),
      name: row.baselineName as string,
    }));
  ...
  if (modelType === "churn") {
    ...
    if (testRow) {
      const calibration = testRow.calibrationJson as
        | ModelPerfEntry["calibration"]
        | null;
      const confusion = testRow.confusionMatrixJson as
        | ModelPerfEntry["confusion"]
        | null;
      const liftTable = testRow.liftTableJson as
        | ModelPerfEntry["lift_table"]
        | null;
      if (calibration) {
...
export const modelPerformanceRoutes = new Elysia({
  prefix: "/model-performance",
})
  .use(requireUser)
  .get("/", async (): Promise<ModelPerfEntry[]> => {
    const built = await Promise.all(MODEL_TYPES.map((t) => buildEntry(t)));
    return [LIFECYCLE_ENTRY, ...built.filter((entry) => entry !== null)];
  })
  // All trained versions for a model type — fuels the production-override picker.
  .get(
    "/:modelType/versions",
    async ({ params, set }): Promise<ModelVersionSummary[]> => {
      if (!isModelType(params.modelType)) {
        set.status = 400;
        return [];
      }
