...
async function fetchTrainingRun(id: string): Promise<TrainingRunRow | null> {
  ...
  const rows = await db
    ...
    .leftJoin(
      trainDataSources,
      eq(mlTrainingRuns.sourceId, trainDataSources.id)
    )
...
const adminTrainingRoutes = new Elysia().use(requireAdmin).post(
  "/",
  async ({ body, userId, set }) => {
    ...
    const [source] = await db
      ...
      .limit(1);
    if (!source) {
      return denyNotFound(set, "Train data source not found");
    }
    ...
    const suggested = await getTrainCutoffSuggestion(
      body.train_source_id,
      horizonDays
    );
    ...
    try {
      await triggerMlJob("/internal/training-runs", {
        training_run_id: inserted.id,
      });
    } catch (e) {
      await db
        .update(mlTrainingRuns)
        .set({ errorMessage: (e as Error).message, status: RUN_STATUS.FAILED })
...
export const trainingRunRoutes = new Elysia({ prefix: "/training-runs" })
  ...
  .delete(
    ...
    async ({ params, userId, isAdmin, set }) => {
      const run = await fetchTrainingRun(params.id);
      if (!run) {
        return denyNotFound(set, "Training run not found");
      }
      if (!isAdmin && run.createdBy !== userId) {
        set.status = 403;
        return {
          message:
            "Only the creator of this training run or an admin can delete it.",
        };
      }
