...
import { type RunRow, runSelect } from "./_helpers";

async function churnThresholds(): Promise<{
  medium: number;
  high: number;
  critical: number;
}> {
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(mlModelAliases.modelType, "churn")
      )
    )
    .limit(1);
...
async function buildSummary(
  run: RunRow & { modelVersionsJson: unknown }
): Promise<RunSummary> {
  ...
  return {
    churn: {
      ...
      eligible_count: agg.churn_eligible,
      thresholds,
    },
    credit: {
    ...
    model_versions:
      (run.modelVersionsJson as RunSummary["model_versions"] | null) ??
      EMPTY_MODEL_VERSIONS,
    revenue: {
...
export const summaryRoutes = new Elysia().use(requireUser).get(
  "/:id/summary",
  async ({ params, set }) => {
