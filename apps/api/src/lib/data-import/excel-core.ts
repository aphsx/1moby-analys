...
export function trimHeader(value: unknown): string | null {
  // biome-ignore lint/suspicious/noEqualsToNull: value is `unknown` from a raw XLSX row and can genuinely be undefined, not just null.
  if (value == null) {
    return null;
  }
...
export function rowIsEmpty(values: unknown[]): boolean {
  for (const v of values) {
    // biome-ignore lint/suspicious/noEqualsToNull: v is `unknown` from a raw XLSX row and can genuinely be undefined, not just null.
    if (v == null) {
      continue;
    }
...
export function validateWorkbookSheets(
  ...
  const _expected = new Set(Object.keys(sheetConfig));
...
export function parseSheetRows(
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    if (skipEmpty && rowIsEmpty(values)) {
      continue;
    }
    out.push({
      excel_row: excelRow,
      row_payload: buildPayload(headers, values),
    });
  }
  return out;
}
